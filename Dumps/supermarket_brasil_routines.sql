...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `ordersperstateview` AS select `supermarket_info`.`State` AS `State`,count(distinct `supermarket_info`.`Order_Id`) AS `NumberOfOrders` from `supermarket_info` group by `supermarket_info`.`State` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
BEGIN
    CREATE TABLE IF NOT EXISTS ProductInfo AS -- LOAD: Creates new table `ProductInfo` and loads the result of the select in it
    SELECT
        ...
        MONTHNAME(orders.order_purchase_timestamp) AS OrderMonth, -- TRANSFORM: Transforms the date timestamp into the month name
        ...
        UPPER(TRIM(payments.payment_type)) AS PaymentType, -- TRANSFORM: Converts payment types to uppercase and removes leading and trailing spaces
...
-- Dump completed on 2023-11-11 23:19:00
